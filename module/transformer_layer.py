...
class MultiHeadedSelfAttention_TDC_gra_sharp(nn.Module):
    ...
    def forward(self, x, gra_sharp):    # [B, 4*4*40, 128]
        ...
        * split D(dim) into (H(n_heads), W(width of head)) ; D = H * W
        ...
        [B, P, C]=x.shape #128 = [1, 640, 96], 64 = [1, 160, 96], 32 = [1, 40, 96]
        # print("Transformer Layer shape:", x.shape)
        try:
            x = x.transpose(1, 2).view(B, C, P//16, 4, 4)      # [B, dim, 40, 4, 4]
        except:
            x = x.transpose(1, 2).view(B, C, -1, 2, 2)      # [B, dim, 40, 4, 4]
        # print("Transformer Layer shape(transpose):", x.shape)
